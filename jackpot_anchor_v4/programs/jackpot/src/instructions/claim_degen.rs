...
pub struct ClaimDegenCandidate<'info> {
    #[account(mut)]
...
pub fn handler(
    ctx: Context<ClaimDegenCandidate>,
    round_id: u64,
    candidate_rank: u8,
    token_index: u32,
) -> Result<()> {
    ...
    degen_claim.set_selected_candidate_rank(candidate_rank);
    degen_claim.set_fallback_reason(DEGEN_FALLBACK_REASON_NONE);
