...
pub fn process_anchor_bytes(
    ...
    let _round_id = parse_round_id_ix(ix_data, "cancel_round")
        .map_err(|_| ProgramError::InvalidInstructionData)?;

    let config = ConfigView::read_from_account_data(config_account_data)
        .map_err(|_| ProgramError::InvalidAccountData)?;
    let mut round = RoundLifecycleView::read_from_account_data(round_account_data)
        .map_err(|_| ProgramError::InvalidAccountData)?;
    ...
    if round.total_usdc != refund_amount {
        return Err(JackpotCompatError::CancelNotAllowed.into());
    }
    ...
    RoundLifecycleView::bit_sub_in_account_data(
        round_account_data,
        participant.index as usize,
        cancelled_tickets,
    )
    .map_err(map_layout_err)?;
    ...
    round.total_tickets = round
        .total_tickets
        .checked_sub(cancelled_tickets)
        .ok_or::<ProgramError>(JackpotCompatError::MathOverflow.into())?;
    if round.total_usdc == 0 {
        round.status = ROUND_STATUS_CANCELLED;
    }
    round
...
mod tests {
    ...
    use crate::{
        ...
        legacy_layouts::{
            ...
            PARTICIPANT_ACCOUNT_LEN, ROUND_ACCOUNT_LEN, ROUND_STATUS_CANCELLED, ROUND_STATUS_OPEN,
    ...
    fn sample_round(round_id: u64, status: u8, vault_pubkey: [u8; 32]) -> [u8; ROUND_ACCOUNT_LEN] {
    ...
    #[test]
    fn applies_cancel_round_and_zeroes_participant() {
        let user = [7u8; 32];
        let round = [4u8; 32];
        let vault = [8u8; 32];
        let usdc_mint = [9u8; 32];
        let config = sample_config([1u8; 32], usdc_mint);
        let mut round_data = sample_round(81, ROUND_STATUS_OPEN, vault);
        ...
        assert_eq!(round_view.total_usdc, 0);
        assert_eq!(round_view.total_tickets, 0);
    ...
    #[test]
    fn rejects_cancel_when_other_deposits_exist() {
        let user = [7u8; 32];
        let round = [4u8; 32];
        let vault = [8u8; 32];
        let usdc_mint = [9u8; 32];
        ...
        let view = RoundLifecycleView {
            total_usdc: 2_000_000,
            ..RoundLifecycleView::read_from_account_data(&round_data).unwrap()
        };
        view.write_to_account_data(&mut round_data).unwrap();
        let mut participant = sample_participant(round, user, 1_000_000);
        let vault_data = token_account(usdc_mint, round);
        let user_ata = token_account(usdc_mint, user);
        ...
        .unwrap_err();
        assert_eq!(err, JackpotCompatError::CancelNotAllowed.into());
    }
}
