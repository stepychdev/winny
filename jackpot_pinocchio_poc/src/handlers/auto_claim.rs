...
pub fn process_anchor_bytes(
    ...
    let _round_id =
        parse_round_id_ix(ix_data, "auto_claim").map_err(|_| ProgramError::InvalidInstructionData)?;
    ...
    let winner_pubkey =
        RoundLifecycleView::read_winner_from_account_data(round_account_data).map_err(map_layout_err)?;
    ...
    let reimburse_vrf = has_vrf_payer
        && vrf_payer_usdc_ata_data
            .and_then(|data| {
                TokenAccountCoreView::read_from_account_data(data)
                    .ok()
                    .and_then(|ata| (ata.mint == config.usdc_mint && ata.owner == vrf_payer_key).then_some(()))
            })
            .is_some();
...
mod tests {
    ...
    #[test]
    fn applies_auto_claim_and_marks_round_claimed() {
